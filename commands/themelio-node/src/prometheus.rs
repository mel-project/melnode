...
use rweb::{get, serve};
use systemstat::{System, Platform, Memory, CPULoad};
use systemstat::platform::PlatformImpl;
use prometheus::{Encoder, Gauge, IntGauge, labels, opts, register_gauge, register_int_gauge, Registry, TextEncoder};
...
static UPTIME_SECONDS: Lazy<IntGauge> = Lazy::new(|| register_int_gauge!(opts!(
...
static MEMORY_TOTAL_BYTES: Lazy<IntGauge> = Lazy::new(|| register_int_gauge!(opts!(
...
static MEMORY_FREE_BYTES: Lazy<IntGauge> = Lazy::new(|| register_int_gauge!(opts!(
...
static NETWORK_TRANSMITTED_BYTES: Lazy<IntGauge> = Lazy::new(|| register_int_gauge!(opts!(
...
static NETWORK_RECEIVED_BYTES: Lazy<IntGauge> = Lazy::new(|| register_int_gauge!(opts!(
...
static ROOT_FILESYSTEM_TOTAL_BYTES: Lazy<IntGauge> = Lazy::new(|| register_int_gauge!(opts!(
...
static ROOT_FILESYSTEM_FREE_BYTES: Lazy<IntGauge> = Lazy::new(|| register_int_gauge!(opts!(
...
static CPU_LOAD_USER: Lazy<Gauge> = Lazy::new(|| register_gauge!(opts!(
...
static CPU_LOAD_SYSTEM: Lazy<Gauge> = Lazy::new(|| register_gauge!(opts!(
...
static CPU_LOAD_IDLE: Lazy<Gauge> = Lazy::new(|| register_gauge!(opts!(
...
fn metrics() -> Result<String, rweb::http::Error> {
    ...
    text_encoder.encode(&REGISTRY.gather(), &mut encoded_output)
    ...
    let output_string: String = match String::from_utf8(encoded_output) {
        Ok(output) => output,
        Err(error) => {
            log::error!("Metrics could not be made into a string from UTF8: {}", error);
...
fn set_highest_block() {
    let storage: &SharedStorage = GLOBAL_STORAGE.get().expect("Could not get a lock on GLOBAL_STORAGE");

    let current_block_count: u64 = storage.read().highest_height();
...
fn set_system_metrics() {
    ...
    let default_network_interface: String = default_net::get_default_interface_name().expect("Could not get default network interface name.");
    ...
    match system.network_stats(&default_network_interface) {
        ...
        Err(error) => log::debug!("There was an error retrieving network traffic information: {}", error),
    }
    ...
    match system.mounts() {
        Ok(mounts) => {
            mounts.iter().for_each(|mount| {
                match mount.fs_mounted_on.as_ref() {
                    "/" => {
        ...
        Err(error) => log::debug!("There was an error retrieving filesystem information: {}", error),
    }
}
...
pub async fn prometheus() {
    ...
    REGISTRY.register(Box::new(HIGHEST_BLOCK.clone())).expect("Adding HIGHEST_BLOCK to the prometheus registry failed.");

    REGISTRY.register(Box::new(UPTIME_SECONDS.clone())).expect("Adding UPTIME_SECONDS to the prometheus registry failed.");

    REGISTRY.register(Box::new(MEMORY_TOTAL_BYTES.clone())).expect("Adding MEMORY_TOTAL_BYTES to the prometheus registry failed.");

    REGISTRY.register(Box::new(MEMORY_FREE_BYTES.clone())).expect("Adding MEMORY_FREE_BYTES to the prometheus registry failed.");

    REGISTRY.register(Box::new(NETWORK_TRANSMITTED_BYTES.clone())).expect("Adding NETWORK_TRANSMITTED_BYTES to the prometheus registry failed.");

    REGISTRY.register(Box::new(NETWORK_RECEIVED_BYTES.clone())).expect("Adding NETWORK_RECEIVED_BYTES to the prometheus registry failed.");

    REGISTRY.register(Box::new(ROOT_FILESYSTEM_TOTAL_BYTES.clone())).expect("Adding ROOT_FILESYSTEM_TOTAL_BYTES to the prometheus registry failed.");

    REGISTRY.register(Box::new(ROOT_FILESYSTEM_FREE_BYTES.clone())).expect("Adding ROOT_FILESYSTEM_FREE_BYTES to the prometheus registry failed.");

    REGISTRY.register(Box::new(CPU_LOAD_USER.clone())).expect("Adding CPU_LOAD_USER to the prometheus registry failed.");

    REGISTRY.register(Box::new(CPU_LOAD_SYSTEM.clone())).expect("Adding CPU_LOAD_SYSTEM to the prometheus registry failed.");

    REGISTRY.register(Box::new(CPU_LOAD_IDLE.clone())).expect("Adding CPU_LOAD_IDLE to the prometheus registry failed.");
