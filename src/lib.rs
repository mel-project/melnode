...
async fn run_node(opt: NodeConfig) {
    ...
    if let Some(sh_no) = opt.test_stakeholder {
        Task::spawn(test_stakeholder(sh_no, auditor.clone(), storage.clone())).detach();
    }

    // Storage syncer
    loop {
        Timer::new(Duration::from_secs(600)).await;
        {
            let storage = storage.clone();
            smol::unblock!(storage.write().sync());
        }
    }
}
...
async fn test_stakeholder(sh_no: usize, auditor: Auditor, storage: Arc<RwLock<Storage>>) {
    ...
    // block forever now
    loop {
        Timer::new(Duration::from_secs(10000000)).await;
    }
}
...
async fn test_spam_txx(auditor: Auditor) {
    ...
    //let txx = &txx[1..];
    for tx in txx {
        Timer::new(Duration::from_millis(1000)).await;
        auditor
